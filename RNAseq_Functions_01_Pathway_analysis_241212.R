...
addGeneFoldChange <- function(enrichresult, fc_data = top_genes_filt, top_genes = 10){
    ...
    fc_data$entrez <- as.character(fc_data$entrez)
    
    
    ### fold change as entrez id to keep data ###
    
    x <- x %>% separate_rows(geneID, sep = "/") %>% 
        left_join(fc_data %>% dplyr::select(entrez, SYMBOL,log2FoldChange) %>% distinct(entrez, .keep_all = T), 
                  by = c('geneID'='entrez')) %>% 
        group_by(ID) %>% 
...
GseaTablesToUpDown <- function(list){
    ...
    for(i in 1:length(list)){
        ...
        sigList[[up_name]] <- t %>% dplyr::filter(NES > 0)
        sigList[[dn_name]] <- t %>% dplyr::filter(NES < 0) 
...
pathwaybarplot <- function(pw.df, 
    ...
    p <- ggbarplot(pw.df, x = 'Description', y = 'FoldEnrichment', fill = 'p.adjust',
                   xlab = '',ylab = 'FoldEnrichment', width = 0.7) +
        coord_flip() +
        ...
        theme(axis.text = element_text( color= 'black'),
              ...
              legend.key = element_rect(color = 'black', fill = NA, linewidth = 0.5),
